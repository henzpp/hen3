...
--인당 평균 구매금액 -2014년 - 1,2,4분기
CREATE TABLE AVG_AMT_2014 
AS SELECT CUSTNO 고객번호, ROUND(AVG(PURAMT)) 평균
FROM PURPRD
WHERE TO_CHAR(TO_DATE(purprd.purdate),'yyyymm')>'201401'
AND TO_CHAR(TO_DATE(purprd.purdate),'yyyymm') < '201501'
GROUP BY CUSTNO
ORDER BY CUSTNO;

DROP TABLE AVG_AMT_2014;

SELECT * FROM AVG_AMT_2014;
...
CREATE TABLE AVG_AMT_2014_124
AS SELECT CUSTNO 고객번호, SUM(PURAMT) 합계
FROM PURPRD
WHERE TO_CHAR(TO_DATE(purdate), 'YYYYMM') >= '201401'
AND TO_CHAR(TO_DATE(purdate), 'YYYYMM') < '201407'
AND TO_CHAR(TO_DATE(purdate), 'YYYYMM') >= '2014010'
GROUP BY CUSTNO
ORDER BY CUSTNO;

DROP TABLE AVG_AMT_2014_124;
SELECT * FROM AVG_AMT_2014_124;

SELECT 고객번호, AVG(합계)
FROM AVG_AMT_2014_124
GROUP BY 고객번호;

--고객별 3분기 구매금액 -2014년
CREATE TABLE AVG_AMT_2014_3
AS SELECT CUSTNO 고객번호, SUM(PURAMT) 합계
FROM PURPRD
WHERE TO_CHAR(TO_DATE(purdate), 'YYYYMM') > '201406'
AND TO_CHAR(TO_DATE(purdate), 'YYYYMM') < '2014010'
GROUP BY CUSTNO
ORDER BY CUSTNO;

DROP TABLE AVG_AMT_2014_3;
SELECT * FROM AVG_AMT_2014_3;
>>>>>>> Stashed changes
